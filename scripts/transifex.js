...
async function getLanguages() {
  const result = await transifexRequest('/api/2/project/scriptsnake-nex/?details');
  return result.teams;
}
...
async function loadRemote(lang) {
  ...
  const result = await transifexRequest(`/api/2/project/scriptsnake-nex/resource/messagesjson/translation/${lang}/?mode=onlytranslated`);
...
async function pushTranslations(lang) {
  ...
  if (Object.keys(remoteUpdate).length) {
    const strings = await transifexRequest(`/api/2/project/scriptsnake-nex/resource/messagesjson/translation/${lang}/strings/`);
    ...
    await transifexRequest(`/api/2/project/scriptsnake-nex/resource/messagesjson/translation/${lang}/strings/`, {
      method: 'PUT',
      responseType: 'text',
      data: updates,
    });
