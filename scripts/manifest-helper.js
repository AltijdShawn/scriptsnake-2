...
async function buildManifest() {
  ...
  if (process.env.TARGET === 'selfHosted') {
    data.browser_specific_settings.gecko.update_url = 'https://raw.githubusercontent.com/scriptsnake/scriptsnake/updates/updates.json';
  }
  if (isBeta()) {
    // Do not support i18n in beta version
    const name = 'scriptsnake BETA';
    data.name = name;
